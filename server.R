...
#This code renders the table and allows the user to select the columns, fifth tab first table
  output$table51 = renderDataTable({
    columns = names(data1)
    if (!is.null(input$select5)) {
      columns = input$select5
    }
    data1[,columns,drop=FALSE]
  })
  
# Downloadable csv of entire dataset, fifth tab, first download button
